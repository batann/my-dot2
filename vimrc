...
"#}}}
"#{{{ >>>   TEMPLATES
autocmd! BufNewFile *.pro 0r ~/.vim/templates/sklt.pro
...
function FoldsHe()
	...
	execute "normal! i# vim:fileencoding=utf-8:foldmethod=marker\r"
endfunction

command! -nargs=0 YESNO :call YesNo()
function YesNo()
	execute "normal! iTMP_FILE=$(mktemp)\r"
	execute "normal! ihead -n470 .bashrc.aliases | tail -n56 > '$TMP_FILE'\r"
	execute "normal! isource '$TMP_FILE'\r"
	execute "normal! irm '$TMP_FILE'\r"
endfunction
...
"#{{{ >>>   Code snipet dpkg check if not
command! -nargs=0 Chek :call DpkgCHK()
function DpkgCHK()
	let g:package2 = input('Enter dependencies: >>   ')
	let g:package1 = input('Enter Software:     >>   ')
	execute "normal! iDEPS_".g:package1."='".g:package2."'\r"
	execute "normal! ifor pack in ${DEPS_".g:package1."[@]}; do\r"
	execute "normal! idpgk -s $pack >/dev/null 2>&1 \r"
	execute "normal! iif [[ $? == 1 ]]; then\r"
	execute "normal! iecho -e '\033[33mInstalling \033[31m${pack}\033[33m...\033[0m'\r"
	execute "normal! isudo apt install $pack -y >/dev/null 2>&1\r"
